...
It also names the political persona you land closest to.

Every country is just one JSON file. Czech 🇨🇿 and Polish 🇵🇱 ship today; EU 🇪🇺
and USA 🇺🇸 are stubbed for later. Adding a domain is a data task, not a code
change.
...
| Home | Result — compass | Result — horseshoe |
|------|------------------|--------------------|
| ![home](docs/assets/home.png) | ![compass](docs/assets/result-compass.png) | ![horseshoe](docs/assets/result-horseshoe.png) |
...
1. Copy `data/cz.json` to `data/<id>.json`. Translate `ui`, `scale`, `axes`;
   ...
   `personas` (a numeric `coords` for every axis); set the three `views`.
